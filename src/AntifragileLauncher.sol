...
import {AntifragileFloorHook} from "./AntifragileFloorHook.sol";
import {AntifragileToken} from "./AntifragileToken.sol";

/// @title AntifragileLauncher
/// @notice Deploys the token and the Antifragile floor hook, initialises the canonical dynamic-fee pool and
///         seeds its initial full-range liquidity in one transaction. Any mismatch reverts the whole launch.
/// @dev The hook is deployed with the token address, start price and tick spacing PRECOMMITTED into its
///      immutables, so {AntifragileFloorHook._afterInitialize} binds only this exact pool (first-pool capture
///      is impossible). The launcher permanently owns the seeded full-range position: it exposes no decrease,
...
///      Programmable volume fee is the fixed constant below (10 bps platform + 20 bps floor reserve); the
///      launcher cannot deploy a hook that charges more, mutates its owner, or bypasses the volume fee.
contract AntifragileLauncher is IUnlockCallback {
    using SafeERC20 for IERC20;
    using PoolIdLibrary for PoolKey;

    /// @dev Canonical Antifragile fee: 30 bps total = 10 bps platform owner liability + 20 bps floor reserve.
    uint16 internal constant FEE_TOTAL_BPS = 30;
    ...
    function deployAndLaunch(LaunchParams calldata params) external returns (AntifragileToken, AntifragileFloorHook) {
        if (msg.sender != launchWallet) revert NotLaunchWallet();
        if (launched) revert AlreadyLaunched();
        launched = true;

        AntifragileToken newToken =
            new AntifragileToken{salt: params.tokenSalt}(params.name, params.symbol, params.totalSupply, address(this));
        ...
        AntifragileFloorHook newHook = new AntifragileFloorHook{salt: params.hookSalt}(
            poolManager,
            FEE_TOTAL_BPS,
